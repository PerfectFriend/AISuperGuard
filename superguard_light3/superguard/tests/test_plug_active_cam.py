...
class FakeCamera:
    ...
    def latest(self):
        ...
        cv2.putText(frame, f'cam{self.cam_id} #{self.frame_counter}', (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        return frame
...
class FakeActuatorManager:
    ...
    def list_all(self):
        ...
        for name in self.actuators:
            ...
            result.append({'name': name, 'type': 'tuya', 'status': 'ONLINE', 'status_icon': '🟢', 'cameras': cams})
        return result
...
def test_plug_binds_to_active_camera():
    ...
    assert bot.actuator_manager.camera_bindings[2] == ['plug1', 'plug2'], bot.actuator_manager.camera_bindings[2]
