...
class TelegramClient:
    ...
    def call(self, method: str, max_retries: int=3, **kwargs) -> Optional[Dict]:
        ...
        if method == 'getUpdates':
            req_timeout = (req_timeout or 25) + 15
    ...
    def send_photo(self, chat_id: int, photo_bytes: bytes, caption: str='', reply_markup: str=None, parse_mode: str='HTML') -> Optional[Dict]:
    ...
    def edit_message_text(self, chat_id: int, message_id: int, text: str, parse_mode: str='HTML') -> Optional[Dict]:
        return self.call('editMessageText', chat_id=chat_id, message_id=message_id, text=text, parse_mode=parse_mode)

    def edit_message_media(self, chat_id: int, message_id: int, photo_bytes: bytes, caption: str='', parse_mode: str='HTML') -> Optional[Dict]:
        media = json.dumps({'type': 'photo', 'media': 'attach://photo', 'caption': caption, 'parse_mode': parse_mode})
        files = {'photo': ('frame.jpg', photo_bytes, 'image/jpeg')}
        return self.call('editMessageMedia', files=files, chat_id=chat_id, message_id=message_id, media=media)
...
class SuperGuardBot:
    ...
    def cmd_plug(self, ctx: CommandContext):
        ...
        for token in arg.split():
            try:
                ...
                self.tg.send_message(ctx.chat_id, f"❌ Не понял номер розетки: '{token}'. Пример: /plug 1 2 3")
                return
    ...
    def set_active_camera_plugs(self, plug_names: List[str]):
        ...
        if unknown:
            self.tg.send_message(self.config.telegram.chat_id, f"❌ Розетки не найдены: {', '.join(unknown)}. Доступные: {', '.join(available)}")
            return
        ...
        self.tg.send_message(self.config.telegram.chat_id, f'✅ Камера {cam_id} ({cam_name}) → розетки: {plugs_str}')

    def cmd_setlocal(self, ctx: CommandContext):
        keyboard = json.dumps({'inline_keyboard': [[{'text': '🇬🇧 EN', 'callback_data': 'set_lang:en'}, {'text': '🇪🇸 ES', 'callback_data': 'set_lang:es'}, {'text': '🇷🇺 RU', 'callback_data': 'set_lang:ru'}]]})
    ...
    def toggle_auto(self):
        ...
        self.save_settings()
        self.refresh_control_msg()
        self.tg.send_message(self.config.telegram.chat_id, self.tr('auto_on') if self.alarm.auto_mode else self.tr('auto_off'))
        if self.alarm.auto_mode:
            self.tg.send_message(self.config.telegram.chat_id, self.tr('auto_on_detail', n=self.config.detection.auto_resolve_frames))
    ...
    def set_zone(self, zone: Optional[Zone]):
        ...
        self.save_camera_settings()
        self.refresh_control_msg()
        self.tg.send_message(self.config.telegram.chat_id, f"📍 {self.tr('zone_set')}: {self.zone_label()}" if zone else f"📍 {self.tr('zone_off')}")
    ...
    def set_target(self, text: str):
        ...
        if not target.classes and (not target.color_ranges):
            self.tg.send_message(self.config.telegram.chat_id, f"🔍 {self.tr('target_set')}: {text}\n{self.tr('target_filter_kept')}: {target.filter_label()}")
        else:
    ...
    def trigger_alarm(self, desc: str, frame: np.ndarray, cam_id: Optional[int]=None, manual: bool=False):
        ...
        reply_markup = json.dumps({'inline_keyboard': [[{'text': '🚨 Отключить тревогу', 'callback_data': 'cancel_alarm'}]]})
        ...
        if manual:
            self.tg.send_message(self.config.telegram.chat_id, self.tr('alarm_on_manual') + '\n' + self.tr('manual_only'))
    ...
    def refresh_control_msg(self):
        if self.alarm.control_msg_id:
            self.tg.edit_message_text(self.config.telegram.chat_id, self.alarm.control_msg_id, self.control_text())
    ...
    def set_bot_menu(self):
        cmds = [{'command': 'autoguard', 'description': self.tr('menu_autoguard')}, {'command': 'togglealarm', 'description': self.tr('menu_togglealarm')}, {'command': 'zone', 'description': self.tr('menu_zone')}, {'command': 'target', 'description': self.tr('menu_target')}, {'command': 'plug', 'description': self.tr('menu_plug')}, {'command': 'setlocal', 'description': self.tr('menu_lang')}, {'command': 'cam', 'description': self.tr('menu_cam')}]
        for lc in ('ru', 'en', 'es'):
    ...
    def handle_update(self, upd: Dict):
        if 'callback_query' in upd:
            ...
            ctx = CommandContext(text=text, args=text[len(text.split()[0]):] if text else '', message_id=mid or 0, chat_id=m.get('chat', {}).get('id', self.config.telegram.chat_id), user_id=m.get('from', {}).get('id', 0))
        ...
        while True:
            ...
            for cam_id in range(1, 9):
                ...
                if streak[cam_id] >= self.config.detection.require_frames and (not self.alarm.is_cam_active(cam_id)):
                    m = matches[0]
                    desc = f"{self.tr('yellow_found')}({m.name} conf={m.confidence:.2f}, color={m.color_fraction * 100:.0f}%)"
